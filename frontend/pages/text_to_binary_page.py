"""
Text to Binary conversion page.
"""
...
from backend.routes.text_to_binary import convert_text_to_binary


class TextToBinaryPage(QWidget):
    """
    Page for converting text to binary representation.
    """

    logout_requested = pyqtSignal()

    def __init__(self, parent=None):
        """Initialize the Text to Binary page."""
        super().__init__(parent)

        layout = QVBoxLayout()
        layout.setSpacing(15)

        title = QLabel("Text to Binary Converter")
        title.setStyleSheet("""
            QLabel {
                font-size: 20px;
                font-weight: bold;
    ...
    def perform_conversion(self):
        ...
        try:
            result = convert_text_to_binary(user_input)
