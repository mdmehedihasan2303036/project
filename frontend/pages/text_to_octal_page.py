...
from backend.routes.text_to_octal import convert_text_to_octal


class TextToOctalPage(QWidget):
    """
    Page for converting text to octal representation.
    """

    logout_requested = pyqtSignal()

    def __init__(self, parent=None):
        """Initialize the Text to Octal page."""
        super().__init__(parent)

        layout = QVBoxLayout()
        layout.setSpacing(15)

        title = QLabel("Text to Octal Converter")
        title.setStyleSheet("""
            QLabel {
                font-size: 20px;
                font-weight: bold;
    ...
    def perform_conversion(self):
        """Perform the text to octal conversion."""
        ...
        try:
            result = convert_text_to_octal(user_input)
