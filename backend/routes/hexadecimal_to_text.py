"""
Hexadecimal to Text conversion route.
Converts hexadecimal numbers (ASCII codes) to their text representation.
"""

from backend.utils.validators import validate_hexadecimal_input
from backend.utils.exceptions import ValidationError, OutOfRangeError


def convert_hexadecimal_to_text(user_input: str) -> dict:
    """
    Convert hexadecimal numbers to text characters.
    Numbers out of ASCII range (0-7F) are replaced with '❌'.

    Args:
        user_input: Hexadecimal number(s) separated by spaces or commas
    ...
    return {
        "input": user_input,
        "conversion_type": "hexadecimal_to_text",
        "result": result_text,
        "success": True
    }
